...
const uint16_t camColors[] = {
  /* keep your full color table here; truncated for brevity */
...
// Helper trait: detect presence of get() member (std::shared_ptr-like)
...
// Extract raw pointer: overload for raw pointer return type
template <typename T>
AsyncWebServer* extract_async_server(T ptr, std::enable_if_t<std::is_pointer<T>::value>* = nullptr) {
  return dynamic_cast<AsyncWebServer*>(ptr);
}

// Extract raw pointer: overload for shared_ptr-like return type
template <typename T>
AsyncWebServer* extract_async_server(T sp, std::enable_if_t<!std::is_pointer<T>::value && has_get_member<T>::value>* = nullptr) {
  return dynamic_cast<AsyncWebServer*>(sp.get());
}

// Fallback: unknown type - return nullptr
template <typename T>
AsyncWebServer* extract_async_server(T, ...) {
  return nullptr;
}
...
void MLX90640::setup(){
  ...
  // --- robustly obtain AsyncWebServer* from base_->get_server() ---
  if (this->base_ == nullptr) {
    ESP_LOGW(TAG, "Web server base_ is null; cannot register HTTP handler");
    return;
  }

  // Capture whatever get_server() returns and extract a raw pointer
  // This handles both raw pointer and std::shared_ptr<T> return types.
  auto server_any = this->base_->get_server();
  AsyncWebServer *server = extract_async_server(server_any);

  if (server == nullptr) {
    ESP_LOGW(TAG, "Web server is not AsyncWebServer. Ensure 'web_server:' (ESPAsyncWebServer) is used in YAML or update component for web_server_idf.");
    return;
  }
  ...
  server->on("/thermal-camera", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    // Prefer direct SPIFFS send overload if available in this build
    // Many AsyncWebServer builds provide request->send(SPIFFS, path, type)
    // The following call will compile when that overload exists.
    // If your build doesn't expose that overload for this request type, replace with a streaming response.
